...
if (!($_SESSION['type'] == "p" || $_SESSION['type'] == "m" || ($_SESSION['level'] == "2" && $_SESSION['type'] == "e"))) {
...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['delete']) && $_POST['delete'] == "yes") {
    $id = $k->real_escape_string($_POST['id']);
    $k->query("delete from employees where id = '$id';");
    if (!$k->error) {
        header('Content-Type: application/json');
        echo json_encode(array("error" => FALSE));
        exit();
    } else {
        send_error("employee not found");
    }
}
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['for_adding']) && $_POST['for_adding'] == "yes") {
    ...
    if (
        ...
        $res = $k->query("SELECT * FROM employees Where email = '$email' and id <> '$id';");
        ...
        if (!$error) {
            ...
            if (isset($_POST['pass']) && !strlen($_POST['pass']) < 8) {
                $pass = $k->real_escape_string(md5($_POST['pass']));
                $k->query("update employees set password = '$pass' where id = '$id'");
            }
            $k->query("update employees set name = '$name' , email = '$email' , kids = '$kids' 
                , bones = '$bones' , bank = '$bank' , level = '$level' , married = '$married',
            gender = '$gender' , zone_id = '$zone_id' , building_id = '$building_id' , job_id = '$job_id' where id = '$id' ;");
            if (!$k->error) {
                $added = TRUE;
            }
        }
...
if (isset($_REQUEST['id']) &&  strlen($_REQUEST['id']) != 0) {
    ...
    if ($_SESSION['type'] == "p") {
        $res = $k->query("SELECT 
        t1.id , t1.name , t1.email , t1.kids , t1.bones , t1.bank , t1.level , t1.married , t1.gender , 
        ...
        WHERE
        t1.job_id = t2.id and 
        t5.minister_id = '$tempId' and
        t1.building_id = t3.id and 
        t1.zone_id = t4.id and 
        t3.ministry_id = t5.id and t1.id = '$id';");
    } else if ($_SESSION['type'] == "e") {
        $tempId = $_SESSION['building'];
...
<body class="sidebar-mini sidebar-closed sidebar-collapse">
                        ...
                        <form id="add_ministry" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                            <div class="row">
                                ...
                                <div class="col-12 mb-3">
                                    ...
                                    <input value="<?php echo $email ?>" placeholder="Enter Email here." name="email" type="email" class="form-control">
                                </div>
                                <input value="<?php echo $id ?>" name="id" type="text" hidden />
                                ...
                                <div class="col-12 mb-3">
                                    ...
                                    <input value="<?php echo $kids ?>" placeholder="Enter kids here." value="0" type="number" name="kids" class="form-control">
                                </div>
                                <div class="col-12 mb-3">
                                ...
                                <div class="col-12 mb-3">
                                    ...
                                    <input value="<?php echo $bank ?>" placeholder="Enter bank money here." value="0" type="number" name="bank" class="form-control">
                                </div>
                                <div class="col-12 mb-3">
                                ...
                                <div class="col-12 mb-3">
                                    ...
                                    <select name="married" class="form-control">
                                        <option <?php if ($married == "yes") {
                                                    echo "selected";
                                                } ?> value="yes">yes</option>
                                        <option <?php if ($married == "no") {
                                                    echo "selected";
                                                } ?> value="no">no</option>
                                    </select>
                                </div>
                                ...
                                <div class="col-12 mb-3 position-relative">
                                    <span style="margin-bottom: 100px;">Zone: <span id="zone_name" class="text-primary"><?php echo $zone_name; ?></span> </span>
                                    <input hidden id="idOf_zone_id" value="<?php echo $zone_id ?>" type="text" name="zone_id" class="form-control">
                                    <button id="select_zone" type="button" class="btn btn-sm btn-primary position-absolute mr-3" style="right: 0px;">Select</button>
                                </div>
                                <div class="col-12 mb-3 position-relative">
                                    <span style="margin-bottom: 100px;">Building: <span id="building_name" class="text-primary"><?php echo $building_name; ?></span> </span>
                                    <input hidden id="idOf_building_id" value="<?php echo $building_id ?>" type="text" name="building_id" class="form-control">
                                    <button id="select_building" type="button" class="btn btn-sm btn-primary position-absolute mr-3" style="right: 0px;">Select</button>
                                </div>
                                <div class="col-12 mb-3 position-relative">
                                    <span style="margin-bottom: 100px;">Job: <span id="job_name" class="text-primary"><?php echo $job_name; ?></span> </span>
                                    <input hidden id="idOf_job_id" value="<?php echo $job_id ?>" type="text" name="job_id" class="form-control">
                                    <button id="select_job" type="button" class="btn btn-sm btn-primary position-absolute mr-3" style="right: 0px;">Select</button>
                                </div>
                                <button id="addmm" type="button" class="btn btn-primary ml-auto mr-auto">Update</button>
                                <button type="button" onclick="deleteMe()" class="btn btn-danger ml-auto mr-auto">Delete</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    <script>
        ...
        $(() => {
            ...
            <?php if ($added) { ?>
                setTimeout(() => {
                    err_m("employee updated successfully.")
                }, 1000);
            <?php } ?>
