...
$number = $page * 20;
if ($_SESSION['type'] == "p") {
    $res = $k->query("SELECT t1.name , t1.id , t2.name job_name FROM employees t1 join jobs t2 WHERE t1.job_id = t2.id limit $number,20;");
} else if ($_SESSION['type'] == "m") {
    $tempId = $_SESSION['id'];
    $res = $k->query("SELECT t1.name , t1.id , t2.name job_name FROM employees t1 join jobs t2 WHERE t1.job_id = t2.id and t2.ministry_id in(select id from ministry where minister_id ='$tempId') limit $number,20;");
}
$number++;

if ($_SESSION['type'] == "p") {
    $num_all = $k->query("select count(*) as 'all' from employees;");
} else if ($_SESSION['type'] == "m") {
    $tempId = $_SESSION['id'];
    $num_all = $k->query("SELECT count(*) as 'all' FROM employees t1 join jobs t2 WHERE t1.job_id = t2.id and t2.ministry_id in(select id from ministry where minister_id ='$tempId')");
}
                            ...
                            <tbody id="table_body">
                                ...
                                <?php while ($fetch = $res->fetch_assoc()) { ?>
                                    <tr>
                                        ...
                                        <td><button onclick="window.location.href = 'employeesUpdate.php?id=<?php echo $fetch['id']; ?>';" class="btn btn-primary btn-sm">Detail</button></td>
                                    </tr>
                                <?php $number++;
                                } ?>
