...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['updateZoneName']) && $_POST['updateZoneName'] == "yes") {
    if (isset($_POST['id']) && isset($_POST['name']) && strlen($_POST['id']) != 0 && strlen($_POST['name']) != 0) {
        $id = $k->real_escape_string($_POST['id']);
        $name = $k->real_escape_string($_POST['name']);

        $res = $k->query("select id from zone where name = '$name' and id <> '$id';");

        if ($res->num_rows != 0) {
            send_error("this name already exsits");
        }
        $res = $k->query("update zone set name = '$name' where id = '$id';");
...
$res = $k->query("select name , id from zone limit $number,20;");
$number++;



$num_all = $k->query("select count(*) as 'all' from zone;");

$num_all_rows_save;
while ($num_all_rows = $num_all->fetch_assoc()) {
    $num_all_rows_save = $num_all_rows['all'];
}
...
<body class="sidebar-mini sidebar-closed sidebar-collapse">
            ...
            <div class="pt-2 pb-2">
                <div class="card ml-2 mr-2 " style="box-shadow: 1px 1px 3px rgb(110, 110, 110) ;">
                    <div class="card-body text-center">
                        <h5 class="card-title mb-4">Zones</h5>
                            ...
                            <tbody id="table_body">
                                ...
                                <?php while ($fetch = $res->fetch_assoc()) { ?>
                                    <tr id="rowId-<?php echo $fetch['id']; ?>">
                                        ...
                                        <td><button onclick="UpdateId(<?php echo $fetch['id']; ?>)" class="btn btn-primary btn-sm">Update</button></td>
                                        <td><button onclick="deleteId(<?php echo $fetch['id']; ?>)" class="btn btn-danger btn-sm">Delete</button></td>
                                    </tr>
                                <?php $number++;
                                } ?>
    ...
    <div class="modal fade" id="mymodal" style="position: fixed; z-index: 12300;" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <button id="cross_update" type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div id="update_body" class="modal-body">
                    <form id="UpdateZoneForm" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                        <div class="row">
                            <div class="col-12 mb-3">
                                <span style="margin-bottom: 100px;">new Name:</span><br />
                                <input placeholder="Enter Name here." name="name" type="text" class="form-control">
                            </div>

                            <input hidden value="yes" name="updateZone" type="text" />
                            <input hidden id="setZoneIdForUpdate" value="" name="id" type="text" />

                            <button onclick="startUpdate()" type="button" class="btn btn-primary ml-auto mr-auto">Add</button>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    <button id="cancle_update_modal" type="button" class="btn btn-primary" data-dismiss="modal">Cancle</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        function startUpdate() {
            let form = document.forms.UpdateZoneForm;
            let form_data = new FormData(form);

            if (!form_data.get("name") || form_data.get("name").length < 1) {
        ...
        function deleteId(id) {
            if (id != "") {
                Id = id;
                $("#prompt_body").text("are you Sure to delete this zone?");
                $("#promptModal").modal("show");
                $("#yes_prompt_modal").on("click", function(id) {
                    $("#promptModal").modal("hide");

                    $.post("delete_zone.php", {
                        id: Id
                    }, function(r) {
                        if (r.error) {
                            err_m(r.m);
        ...
        $(() => {
            ...
            $("#cancle_update_modal").on("click", function() {
            ...
            $("#cross_update").on("click", function() {
                $("#updateModal").modal("hide");
            })
