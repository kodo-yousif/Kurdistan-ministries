...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['delete']) && $_POST['delete'] == "yes") {
    ...
    $k->query("delete from building where id='$id';");
...
if (isset($_REQUEST['id']) &&  strlen($_REQUEST['id']) != 0) {
    $id = $k->real_escape_string($_REQUEST['id']);
    if ($_SESSION['type'] == "p") {
        $res = $k->query("SELECT t1.id id , t1.bank bank , t1.name building_name , t2.name ministry_name , t2.id ministry_id , t3.id zone_id , t3.name zone_name from building t1 join ministry t2 join zone t3 where t1.ministry_id = t2.id and t1.zone_id = t3.id and t1.id = '$id';");
    } else if ($_SESSION['type'] == "m") {
        $tempId = $_SESSION['id'];
        $res = $k->query("SELECT t1.id id , t1.bank bank , t1.name building_name , t2.name ministry_name , t2.id ministry_id , t3.id zone_id , t3.name zone_name from building t1 join ministry t2 join zone t3 where t1.ministry_id = t2.id and t2.minister_id ='$tempId' and t1.zone_id = t3.id and t1.id = '$id';");
    }
                    ...
                    <div class="card-body ">
                        ...
                        <form id="add_ministry" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                            ...
                            <div style="display: flex; width: 100%; justify-content: center; flex-direction: column;">
                                ...
                                <button type="button" style="width: 77px;" onclick="$('#depo').modal('show');" class="btn btn-secondary ml-auto btn-sm mr-auto mb-2">deposit</button>
                                <button type="button" style="width: 77px;" onclick="$('#with').modal('show');" class="btn btn-sm btn-secondary ml-auto mr-auto mb-2">withdraw</button>
                            </div>
                        </form>
                    </div>
                </div>
