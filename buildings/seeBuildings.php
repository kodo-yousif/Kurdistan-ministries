...
if ($_SESSION['type'] == "p") {
    $res = $k->query("SELECT t1.id 'id' , t1.name 'building_name' , t2.name 'ministry_name' from building t1 join ministry t2 where t1.ministry_id = t2.id limit $number,20;");
} else if ($_SESSION['type'] == "m") {
    $tempId = $_SESSION['id'];
    $res = $k->query("SELECT t1.id 'id' , t1.name 'building_name' , t2.name 'ministry_name' from building t1 join ministry t2 where t1.ministry_id = t2.id and t2.minister_id = '$tempId' limit $number,20;");
}

$number++;

if ($_SESSION['type'] == "p") {
    $num_all = $k->query("select count(*) as 'all' from building;");
} else if ($_SESSION['type'] == "m") {
    $tempId = $_SESSION['id'];
    $num_all = $k->query("select count(*) as 'all' from building where ministry_id in (select id  from ministry where minister_id = '$tempId');");
}
            ...
            <div class="pt-2 pb-2">
                <div class="card ml-2 mr-2 " style="box-shadow: 1px 1px 3px rgb(110, 110, 110) ;">
                    <div class="card-body text-center">
                        <h5 class="card-title mb-4">Buildings</h5>
                            ...
                            <tbody id="table_body">
                                ...
                                <?php while ($fetch = $res->fetch_assoc()) { ?>
                                    <tr>
                                        ...
                                        <td><button onclick="window.location.href = 'buildingUpdate.php?id=<?php echo $fetch['id']; ?>';" class="btn btn-primary btn-sm">Detail</button></td>
                                    </tr>
                                <?php $number++;
                                } ?>
