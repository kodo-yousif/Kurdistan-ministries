...
if ($_SESSION['type'] == "p") {
    $res = $k->query("select ministry.id as 'id' ,ministry.name as 'ministry_name', minister.name as 'minister_name' from ministry join minister where minister.id = ministry.minister_id limit $number,20;");
} else {
    $tempId = $_SESSION['id'];
    $res = $k->query("select ministry.id as 'id' ,ministry.name as 'ministry_name', minister.name as 'minister_name' from ministry join minister where minister.id = ministry.minister_id and ministry.minister_id = '$tempId' limit $number,20;");
}
$number++;


if ($_SESSION['type'] == "p") {
    $num_all = $k->query("select count(*) as 'all' from ministry;");
} else {
    $tempId = $_SESSION['id'];
    $num_all = $k->query("select count(*) as 'all' from ministry where ministry.minister_id = '$tempId' ;");
}
$num_all_rows_save;
                            ...
                            <tbody id="table_body">
                                ...
                                <?php while ($fetch = $res->fetch_assoc()) { ?>
                                    <tr id="rowId-<?php echo $fetch['id']; ?>">
                                        ...
                                        <td><button onclick="window.location.href = 'ministryUpdate.php?id=<?php echo $fetch['id']; ?>';" class="btn btn-primary btn-sm">Detail</button></td>
                                    </tr>
                                <?php $number++;
                                } ?>
