...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['name']) && strlen($_POST['name']) !== 0) {
    ...
    $res = $k->query("select name , id from president where name like '%$name%';");
    ...
    while ($oneRow = $res->fetch_assoc()) {
        ...
        <td>$name</td>
        <td><button onclick=\"window.location.href = 'updateAdmin.php?id=$id';\" class=\"btn btn-primary btn-sm\">Detail</button></td>
