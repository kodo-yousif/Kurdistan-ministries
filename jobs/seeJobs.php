...
if ($_SESSION['type'] == "p") {
    $res = $k->query("select t1.id id , t1.name name, t2.name ministry_name from jobs t1 join ministry t2 where t1.ministry_id = t2.id limit $number,20;");
} else if ($_SESSION['type'] == "m") {
    $tempId = $_SESSION['id'];
    $res = $k->query("select t1.id id , t1.name name, t2.name ministry_name from jobs t1 join ministry t2 where t1.ministry_id = t2.id and t2.minister_id = '$tempId' limit $number,20;");
}
...
if ($_SESSION['type'] == "p") {
    $num_all = $k->query("select count(*) as 'all' from jobs;");
} else if ($_SESSION['type'] == "m") {
    $tempId = $_SESSION['id'];
    $num_all = $k->query("select count(*) as 'all' from jobs where ministry_id in(select id from ministry where minister_id = '$tempId' ) ;");
}
                            ...
                            <tbody id="table_body">
                                ...
                                <?php while ($fetch = $res->fetch_assoc()) { ?>
                                    <tr>
                                        ...
                                        <td><button onclick="window.location.href = 'jobsUpdate.php?id=<?php echo $fetch['id']; ?>';" class="btn btn-primary btn-sm">Detail</button></td>
                                    </tr>
                                <?php $number++;
                                } ?>
