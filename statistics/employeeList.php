...
if ($_SESSION['type'] == "p") {
    ...
    (t1.bones + t2.bones ) + (t1.level*t2.per_level) + (t2.per_month * TIMESTAMPDIFF(MONTH, t1.created_date ,CURRENT_TIMESTAMP ))+
    if(t1.zone_id = t3.zone_id , 0 , t2.outside_zone) ) as 'next', t1.name , t1.id , t2.name job_name FROM employees t1  join building t3 join jobs t2 WHERE t3.id = t1.building_id and  t1.job_id = t2.id and t2.ministry_id in(select id from ministry where minister_id ='$tempId') limit $number,20;");
    ...
    WHERE t1.job_id = t2.id and t3.id = t1.building_id and t1.building_id = '$build_id_s'
    limit $number,20;");
}
...
if ($_SESSION['type'] == "p") {
    ...
    $tempId = $_SESSION['id'];
    $num_all = $k->query("SELECT (t2.start_salary + if(t1.married = 'yes', t2.married_bones , 0 ) + (t1.kids * t2.per_kid) +
    (t1.bones + t2.bones ) + (t1.level*t2.per_level) + (t2.per_month * TIMESTAMPDIFF(MONTH, t1.created_date ,CURRENT_TIMESTAMP ))+
    if(t1.zone_id = t3.zone_id , 0 , t2.outside_zone) ) as 'next', t1.name , t1.id , t2.name job_name FROM employees t1  join building t3 join jobs t2 WHERE t3.id = t1.building_id and  t1.job_id = t2.id and t2.ministry_id in(select id from ministry where minister_id ='$tempId');");
} else if ($_SESSION['type'] == "e" || $_SESSION['level'] = "2") {
    $build_id_s = $_SESSION['building'];
    $num_all = $k->query("SELECT count(t1.id) as 'all' FROM 
    employees t1 join jobs t2 join building t3
    WHERE t1.job_id = t2.id and t3.id = t1.building_id and t1.building_id = '$build_id_s';");
}
                            ...
                            <tbody id="table_body">
                                ...
                                <?php while ($fetch = $res->fetch_assoc()) { ?>
                                    <tr>
                                        ...
                                        <td><?php echo $fetch['job_name'] ?></td>

                                        <td><?php echo number_format($fetch['next']) ?></td>
                                        <td><button onclick="window.location.href = '../employees/employeesUpdate.php?id=<?php echo $fetch['id']; ?>';" class="btn btn-primary btn-sm">Detail</button></td>
                                    </tr>
                                <?php $number++;
                                } ?>
