...
if ($_SESSION['type'] == "p") {
    ...
    SUM(
        ...
        (t1.level*t2.per_level) + (t2.per_month * TIMESTAMPDIFF(MONTH, t1.created_date ,CURRENT_TIMESTAMP ))+ 		if(t1.zone_id = t3.zone_id , 0 , t2.outside_zone)  
    ) as salary
    FROM 
    employees t1 join 
    jobs t2 join 
...
if ($_SESSION['type'] == "p") {
    $num_all = $k->query("select t3.name
        FROM 
        employees t1 join 
        jobs t2 join 
        building t3 join 
